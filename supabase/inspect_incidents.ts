import { createClient } from '@supabase/supabase-js'
import dotenv from 'dotenv'

dotenv.config({ path: 'apps/.env.local' })

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY

const supabase = createClient(supabaseUrl!, supabaseServiceKey!)

async function inspectIncidents() {
  const { data, error } = await supabase.from('incidents').select('*').limit(1)
  if (data && data.length > 0) {
    console.log('Incidents Columns:', Object.keys(data[0]))
  } else {
    // Try to get from metadata or empty select
    const { data: cols, error: err } = await supabase.from('incidents').select().limit(0)
    console.log('Error or Columns:', err || cols)
  }
}
inspectIncidents()
